...
--
-- EVERYTHING IS enabled = false. SP3 turns the first one on deliberately, and
...
('SAM.gov', 'US', 'SAM', '1 api', 'in',
 ...
 ('{"works":["naics","psc","noticeType","sort=-modifiedDate"],'
   ...
   || 'must stop on modifiedDate, since modifiedDate >= publishDate always."}')::jsonb,
 'P7D', false,
...
('Indiana IDOA solicitations', 'IN', 'IDOA static list', '3 html', 'in',
 ...
 ('{"note":"Plain HTML table. No RSS, API or bulk download. Event name, agency, '
   || 'event ID, description, due date, contact."}')::jsonb,
 'P7D', false,
...
('Indiana EDS contract register', 'IN', 'IDOA contract search', '1 api', 'in',
 ...
 ('{"works":["businessUnit","endDate","pageSize","sort=-modifiedDate"],'
   ...
   || 'Stop on modifiedDate instead."}')::jsonb,
 'P7D', false,
...
('Illinois BidBuy', 'IL', 'Periscope S2G', '3 html', 'in',
 ...
 ('{"works":["status","organization","classId","typeCode","categoryCode",'
   ...
   || 'Intent To Award, Opened, Bid to PO, Sent."}')::jsonb,
 'P7D', false,
...
('Michigan SIGMA VSS', 'MI', 'CGI Advantage VSS', '3 html', 'in',
 ...
 ('{"silently_ignored":["Show Me"],'
   ...
   || 'rot signal.",'
   || '"beware":"Award History is GRANT disbursements, not procurement awards."}')::jsonb,
 'P7D', false,
...
('Kentucky eMARS VSS', 'KY', 'CGI Advantage VSS', '3 html', 'in',
 ...
 ('{"note":"INFERRED FROM PLATFORM, not tested. Everything here is Michigan''s '
   || 'behaviour assumed to repeat. Verify before relying on it."}')::jsonb,
 'P7D', false,
...
('BidNet Direct', 'US', 'Aggregator', '4 manual', 'out',
 'Paywalled aggregator. Excluded by its terms of service (§5.5). Not accessed.',
 NULL, NULL, NULL, false, 'Standing exclusion.'),
