...
VALUES ('Koehler Partners', 1, 'The firm this instance is configured for. §4.2.');

INSERT INTO vendor_alias (vendor_id, alias, source_note)
SELECT id, 'KP', 'Internal shorthand.' FROM vendor WHERE is_self = 1;
...
SELECT
  ...
  json('{"naics":["541611","541612","541618","541690","541720","611430"],'
    ...
    || 'these systems develop a silent recall problem."}'),
  json('{"wbe":{"state":"IN","expires":"2027-04"},"mbe":"pending"}'),
  json('{"primary":["IN"],"secondary":["IL","OH","KY"],"federal":true,'
    || '"note":"Scope is a Profile setting, not code (§1A). Illinois matters more since '
    || '2026-08-12: it is the only non-federal source retaining closed solicitations."}'),
  1,
  json('{"headcount":14,"trailing_revenue_usd":2800000,"bonding_capacity":null,'
    ...
    || 'rule binds the machine, not the user."}'),
...
FROM vendor v WHERE v.is_self = 1;
