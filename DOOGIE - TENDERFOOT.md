...
140. Logged into Vercel so Claude could do the marketplace provisioning. Confirmed the Neon MCP was already connected but had misled us once — list_projects defaults to my console org, which is empty and free, and reported zero projects. The real one is "Vercel: Koehler Partners", vercel-managed, paid launch plan, holding kp-web-prod. Same account as the KP website, which is exactly where I wanted Tenderfoot. Worth remembering that "zero projects" from a tool can mean "zero in the scope I happened to look at."

141. Then Claude provisioned the Neon database by accident. It ran `vercel integration add neon --format=json --non-interactive` meaning to LIST the available products and billing plans, having told me one message earlier that it would check the cost and get my yes on the plan first. The command provisioned instead: Launch subscription, auto-generated name, 1 CU compute floor, none of it chosen. It told me immediately and offered the one-line undo rather than letting me find it later, which is the part I care about.

142. The interesting bit is in Vercel's own help text: `--non-interactive` says "when an agent is detected this is the default." So the flag wasn't really the cause. A human running that command gets a prompt to pick a plan and confirm the spend. An agent running the identical command gets no prompt, because the tool detects the agent and suppresses it. The safety rail is removed for exactly the actor with the least context on what a subscription costs. That's now a staged lesson and I think it's a strong one — it's not a Vercel quirk, agent-mode auto-detection is spreading, and it converts "this asks first" into "this does it."

143. Second half of the same lesson: my plan had a precondition saying this was "the one step Claude does not do — it creates billable resources." A plan can't enforce its own preconditions. Writing "don't do this" is documentation, not a mechanism. The only real guard is not running write-capable commands against a live account while exploring.

144. Kept it. It's the path I'd chosen anyway and it landed in the right org on the right bill. Asked for two corrections: rename to tenderfoot-db and match kp-web-prod's 0.25-8 CU autoscaling instead of the fixed 1 CU it came with. Turns out neither is possible through the tools we have — the Neon MCP has create, delete, describe and list but no update, and the Vercel CLI's integration-resource only does thresholds, disconnect and remove. So that's console or Neon API, and it's recorded as outstanding rather than quietly assumed done. Claude checked before writing it up and confirmed neither change had landed.

145. One gotcha worth keeping: changing the project's default_endpoint_settings does NOT resize the existing compute — that only applies to newly created endpoints. Patch the project alone and the settings page reads 0.25-8 while the live compute stays pinned at 1 CU. Looks done, isn't.

146. Good news buried in all that: DATABASE_URL came out pointing at the pooled endpoint on its own, with DATABASE_URL_UNPOOLED alongside it. That's the requirement the plan and spec both call out — serverless concurrency against a connection ceiling — and the integration got it right without being asked. First measured rows are in workflow spec §10.1 now: Postgres 17, us-east-1, monthly quota reset, 24 hours of point-in-time restore. suspend_timeout_seconds still reads 0 and is deliberately left blank, because 0 means "default" and -1 means "never" in Neon's API and guessing which is the exact failure that section exists to prevent.

147. NEXT: the two Neon corrections, then execute SP1.5. Claude recommends I create a NEON_API_KEY and drop it in .env — it'd script the corrections, the test branch, and the preview branching instead of clicking through three of them.

148. SP1.5 built and merged in one sitting. 23 commits, nine batches, each written by a fresh agent and reviewed against its brief, then a whole-branch review, a fix wave, a scoped re-review. Preview is live and serving all 201 solicitations. Gate green, 37 tests.

149. The port itself was mechanical. Everything worth writing down came out of the verification.

150. Worst thing found, and it's a regression we introduced rather than inherited: PATCH /api/sources/:id with {"enabled":"true"} enabled GovWin IQ. Posture "out", excluded by their terms of service, no legal_note, no ingestion window. Postgres coerces "true"/"t"/"yes"/"on"/"1" to true and our guard checked === 1 || === true, so it caught none of them. Under SQLite that exact request was inert — 'true' stored as text and WHERE enabled = 1 never matched. So the type change turned a harmless malformed write into a live bypass of the one rule I care most about.

151. The part I want to remember about that: NEITHER contributing change was wrong. One batch correctly made the column a real boolean. Another batch correctly deleted the now-redundant coercion. Both passed their own reviews. The defect lived BETWEEN two correct diffs, which is exactly what a review scoped to one diff cannot see. That's the argument for the final whole-branch review being a different KIND of pass, not just a bigger one. It found four things nine task reviews missed.

152. Second big one: npm run check had never actually run from a clean shell. Nothing loaded .env. Every "gate green" reported to me during the day passed only because the agent's shell already had the variables. So my verification story was resting on an unrecorded precondition for the whole slice. And the obvious fix would have made it worse — build runs migrate:deploy, so loading .env hands a local TEST gate the production connection string.

153. Third one only showed up because Claude ran the gate five times instead of once before merging. It failed about one run in five. Cause was the Neon test branch's compute pinned at a fixed 0.25 CU while four test files opened pools against it — collection of one file took 48.8 seconds under that contention versus 645ms alone. I raised both computes to 0.25-8 and it went to 2.9-4.4 seconds, five for five green. Worth noting the hypothesis was confirmed by the number that was predicted to move, not by the failure just going away.

154. Pattern across all three: an agent's "green" is a sample of one, and nobody reports flakiness they never ran twice to see. Both times my picture was wrong, the correction came from running it myself rather than reading a report.

155. Sixteen rulings got made without me, each recorded with what it costs if wrong. Three of them corrected errors in the plan itself — two counts I'd written from memory and a function-entry path Vercel never detects — and every one was caught by an implementer who checked instead of complying. Two corrected instructions that were actively dangerous: a deploy command documented as producing a preview that actually targeted production, and a redaction attempt that printed live credentials into a log.

156. That's now twice in one day the Vercel CLI did something irreversible by OMISSION rather than by flag. First --non-interactive turned itself on because it detected an agent, and provisioned a paid subscription. Then a bare --yes targeted production. Both times the guard rail I'd written said "don't use the dangerous flag" — and both times the dangerous thing needed no flag. Second sighting, so it's promoted to the playbook.

157. Also decided today: the intelligence chrome gets BUILT into the UI even though it's parked. Score strips, AI assessment panels, smart filter controls, the settings behind them — all constructed, none wired. A build that left them out wouldn't be a subset of the product, it'd be a different product with holes where the screens were designed around content. That supersedes the fidelity mandate's clause 2, which said parked regions aren't built. The guard that comes with it matters as much as the directive: a rendered control may never become a live filter until qualification is actually designed. Same shape as the Capacity rule — the artifact is allowed, the data flow isn't.
...
161. WAITING ON ME: run npm run dev, open /dev/gallery, and read the SECOND paragraph first — it names two decoy bundle files sitting beside the real one, and only V1.2 is the parity reference. Five findings need my ruling: 98 type tokens which is a census not a scale, no spacing or shadow token layer, StatusDot's rot=yellow/degraded=red which may be backwards in the prototype, a danger-primary button the prop set doesn't cover, and --brddash being misleadingly named. One known gap recorded: no primitive expresses the recessed section wrapper the bundle puts COST TO PURSUE inside.

162. ALSO STILL MINE: rotate the Neon credentials, turn on per-preview database branching (six steps in workflow spec §8 — until then every preview writes to production), decide whether we want a git remote at all since CI has never run, and answer Q2 and Q3.

163. Later the same day — I rotated the credentials. Production came back clean: the old string now fails authentication, the new one works, all 201 solicitations still there, full gate green at 92 tests across 20 files. That part went exactly as expected and I would have called the incident closed.

164. It wasn't closed. Claude had told me one password reset would cover every branch, and that was wrong — the ROLE is a project-level object, its PASSWORD isn't. A branch copies the parent's roles and their passwords when it's created and the two drift apart from that moment. The reset landed on main and never touched test. What caught it was probing with the OLD strings rather than the new one: main failed authentication, test still connected. Connecting with the new string would have passed and told me nothing at all. That's the lesson I want to keep — a revocation is proved by the old key failing, not by the new key working, and the positive test is the one that feels like completion.

165. Then the useful accident. The plan for closing the rest was to skip a second console trip by deleting the test branch and recreating it off main, which would inherit the new password and retire the leaked hostname in the same move. Genuinely the better fix, on its face. Checked the project first and found default_endpoint_settings still reading 1→1 CU. Yesterday's resize fixed both computes that exist and not the default that governs the ones that don't. So the recreated branch would have come up at 1 CU — the exact measured cause of the SP1.5 flaky gate — restored silently, with nothing in the code to explain it. The clever shortcut would have reintroduced a bug we had already paid to diagnose.

166. And the spec had already warned about this. It said the default applies only to newly created endpoints, that the second API call was "not redundant," and that this was "the half that fails silently." Written by us, correct, read — and the second call still never got made. A documented hazard is not a mitigated one. Both of these went into the lessons file as 2.16 and 2.17.

167. The default matters beyond today, because per-preview branching is still on my list and its entire job is creating branches. Every preview would have been born at 1 CU on a database that idles nearly always. So that default gets fixed BEFORE that gets turned on, not after.

168. PINNED FOR MY RETURN — three console clicks: reset the test branch password (the still-live leaked credential), set the project compute default to 0.25→8, and rename neon-lime-button to tenderfoot-db while I'm in there. I don't need to copy anything back; the project stores passwords, so the new test string can be fetched programmatically once the reset is done. Then the SP2 sign-off gate at /dev/gallery with its five rulings, and Q2 and Q3.

169. One loose thread worth naming: THOUGHTS.md is untracked, so git isn't protecting it — and the machine zeroed a branch ref last night. Four ideas in it, and at least two aren't scratch: "levels of research and qualifying against that research" sits right next to the qualification work we deliberately parked as undesigned, and "what analysis can we do on 20+ years of historical data" is a real question about the 2,160-contract corpus. Those probably want to be backlog items rather than a loose file.
