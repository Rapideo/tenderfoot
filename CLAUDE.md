...
- **A green server test does not mean the screen works.** SP3.6 passed every server-side test with both its buttons broken in a browser. **Click through it.** The Claude Chrome extension works here (confirmed 2026-08-31); CDP also works.
  ...
  - ⚠️ **A third trap, 2026-09-16: the extension may be talking to a DIFFERENT Chrome.** More than one browser can be connected to the account (a Mac was, that day), and the tools answer from whichever is selected — so `localhost` is *that machine's* localhost, every local page reads as "error page" while `example.com` renders, and it looks exactly like a site-permission block (the 09-15 diagnosis was probably this). **Before any localhost work: `list_connected_browsers`, then `select_browser` on the `isLocal: true` Windows one.** Then prefer `find` + `ref` clicks over coordinates: the window may be resized under you mid-session and coordinates go stale.
