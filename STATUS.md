...
| **SP4** | Fetch + extraction — documents parsed, fields cited | ◐ **ALL TWELVE TASKS BUILT 2026-08-30** on `sp4-fetch-extraction` (pushed; **production is deployed from this branch, not `main`**). Gate **417 tests / 58 files**. Run live against the `test` branch: **79 documents processed, 0 pending**, first accuracy reading `closes_at` agreed 2 / disagreed 0 / missed 14 / opportunities 16 — precision 100%, recall 12.5%. Browser click-through done over CDP. **Demo criterion bullets 2 and 3 DEFERRED TO SP6 by Matt 2026-08-30** (they need a record view SP6 owns — see the spec's §10.1 for what dies with that). **Not yet reviewed, not yet merged, not yet deployed to production** |
