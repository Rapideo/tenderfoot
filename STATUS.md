...
**You are on `main`, everything is pushed, and the working tree is clean.** Gate green at **193 tests / 36 files**, `npm run check` exit 0, and the gate itself now runs in **94s, down from ~154s**. CI green on `main`.
...
1. **`corpus.test.ts` — ✅ MOSTLY FIXED 2026-08-17 (`2cac516`), and read the caveat before closing it.**
