...
>
> **All three Neon console changes are done** — test-branch password, compute default `0.25 → 8`, and the project rename. **Nothing infrastructural is pinned for Matt; two design items are** — the scaffolding brainstorm and §6.4 A3. *(Briefly untrue on 08-15, when per-preview branching turned out to need a Git connection; ruled and executed the same day, so the list is back to two.)*
>
