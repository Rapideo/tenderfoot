...
2. **The deadline labelling task — the highest-value thing a human can do on this slice.** 14 blocks, one per recall miss, each with the portal's value, every document read, and every date-bearing line in context. **The 12.5% recall figure assumes every miss was ours**, and nobody has checked; a first skim suggests several are clean `NOT-IN-DOC`.
