...
**Updated 2026-09-16.** One screen. The reasoning lives elsewhere; this is only where things stand.
...
## ⏸️ START HERE NEXT SESSION — written 2026-09-16 (end of day), at Matt's request

**State on disk: `main == origin/main` at the commit carrying this block. Working tree clean. Gate 1173 tests / 104 files, `npm run check` exit 0 — run on branch tip `e0feab0`, and the `--no-ff` merge `60a7ba3` produced a byte-identical tree (verified `git diff e0feab0 main` empty), so the figure stands for `main` without a second run. Dev servers were shut down deliberately (the npm wrappers AND their child node processes — `TaskStop` alone leaves the children listening; kill by PID). Nothing of ours is running. No API call was made today; the trial's reserve is untouched at 8,682 of 10,000.**

⚠️ **The push deploys.** Vercel's build runs `migrate:deploy`, so **migration 035 (`pursuit.reason_chips`, a column with a default, a CHECK, an index — no data touched) lands in production with this push.** First thing: confirm it on `/api/health`'s migration list, as 034 was confirmed this morning. The queue in production will show the chips the moment it deploys.

**Then, in order — the first four cost nothing:**

1. **Two rulings still waiting from 2026-09-15, both free, both small.** D31's narrowing (the `DOCUMENTS NOT REQUESTED` head only when the row also holds nothing — one condition to drop if he disagrees) and **R7's broken mirror** (D17 removed F6's p10, so `p6DescriptionP10Adequate` mirrors nothing and collapses on thin-listing sources — should be settled before R7 is ratified). Arguments in `docs/admin-deviations.md` D31 and `app/server/src/fitness/thresholds.ts`.
2. **Three follow-ups D32 names as NOT decided by the chip build**, each a small ruling or proposal, none a record:
   - **The negative profile** (spec §4.2 — *"what will never be bid, and why"*, built from exactly these no-bid reasons). `firm_profile.negative_profile` is still empty; the 113 nouns are `SELECT reason FROM pursuit WHERE reason_chips @> '{not-a-service}'` away. Propose: a read-only script that lists them grouped, for Matt to prune into the field — decide from a fillable artifact, not prose.
   - **D2's trigger.** An Interested row wearing *Not enough information* is precisely the row the on-demand document fetch exists for. Whether the chip *triggers* the fetch is a ruling, and a yes costs records per open — §5.1 applies to the design, not just the call.
   - **The record screen shows no decision at all** (verified: no client file outside the queue consumes `decision`). Reason and chips wait there. A §7.10 question first — does the bundle draw a decision on the record? — before anything is built.
3. **Grade HigherGov itself** — `R7 UNKNOWN, 0 of 5 properties measured`. Probably free from the 4,377 rows held.
4. **The corpus will start carrying chips.** `scripts/reason-corpus.mjs` reads `pursuit.reason` and knows nothing of `reason_chips`; the next sample's blocks should print the chips picked beside the reason, so the next derivation starts from what was tapped and what was typed. Small; do it before the next sitting, not after.
5. **Then, when Matt wants it: the subscription decision** ($500/yr; strong on state agencies, silent on sub-state — 2026-09-15 §4 has the caveats).

**Still open from before, unchanged:** the working-set plan's **§5** · the deadline-labelling task (⏸ PARKED by ruling) · F7, which closes itself through use.

**The rest of this block is the day as it was written, newest first.**

---

## 🔖 WHERE THINGS ACTUALLY STAND — 2026-09-16 (morning), written before any work

**On disk: `main == origin/main` at `7f998a9`. The working tree is NOT clean, and the reason is a session this file did not know about.** Between the 22:28 close-out on the 15th and about 01:00 on the 16th, a **management addendum to the explainer** was built and left uncommitted: [`docs/Tenderfoot-Explainer-Addendum.md`](docs/Tenderfoot-Explainer-Addendum.md) (the fuller wording), [`docs/Tenderfoot-Explainer-Addendum.pdf`](docs/Tenderfoot-Explainer-Addendum.pdf) (4 pages, letter, trimmed to fit), and [`docs/addendum/`](docs/addendum/README.md) (`addendum.html`, `build.py`, and a gitignored `proof/` of page PNGs — the `.gitignore` line for `proof/` is the fourth uncommitted change). It is **a status document, not a specification**: dated 16 September, "what the explainer promotes, what the software actually does today, and where the team is", every figure read from this file, DOOGIE, the plan, and the fitness and platform-comparison documents on the day it was built. **Its own README says when to rebuild it** — the go / no-go, the subscription decision, a post-gate slice landing — and to change the date in the band and the footer when you do. Two of its figures were re-verified this morning: **607 commits** (`git rev-list --count HEAD` at `7f998a9`) and the floor at **six of seven** (F7 the last, per the 09-15 entry below). The rest were not re-measured. Its README is render-only and its spend figure is the close-out's own 8,682, so the addendum session cost nothing metered.

**Migration 034 is confirmed in production** — read-only `GET /api/health`, 2026-09-16, `200`, the list ends at `034_document_key.sql`. That closes the "confirm on `/api/health`" ask in the 09-13 block directly below, which had stayed open through two pushes.

**Nothing else moved.** No code has changed since `7f998a9`; the gate was last run there (exit 0, 1150 tests / 104 files) and was not re-run this morning. No API call has been made this morning — §5.1 is in full force, the trial closed at **8,682 of 10,000**, and the remainder is held in reserve by ruling. **The resume list is unchanged: the ordered `START HERE TOMORROW` list in the 09-15 block further down**, whose first item is still the chip derivation with Matt from [`docs/2026-09-13-reason-corpus.md`](docs/2026-09-13-reason-corpus.md). DOOGIE has no 16 September entry; the addendum session left none, and one is Matt's to write or to ask for.

**⚖️ THE CHIP DERIVATION IS DONE AND ON A SHEET — rulings D20–D24: <https://claude.ai/artifact/NdhuQyUot3itr9ciQ7zHC7>** (`read_db`, collection `rulings`, docs `d20`…`d24`; block moves in `d21-roster`, renamed chips in `d21-labels`). All 150 blocks of [`docs/2026-09-13-reason-corpus.md`](docs/2026-09-13-reason-corpus.md) were read and assigned, counts computed by script, not remembered. **What the 150 say:** Pass 139 → *Not a service we provide* **113** (construction & roads 41 · specialty equipment & materials 37 · trades & infrastructure 16 · licensed & provider services 13 · grounds & janitorial 6; "add to the blacklist" ~25×) · *Not enough information* 13 (+4 as a second reason) · *Not an actual bid* 8 · *Seen already* 2 · *Must be on-site* 2 · *Too specific* 1; Interested 11 → *Perfect fit* 2 · *Our kind of work* 5 · *Could source it* 2 · *New market* 1 · *Not enough information* 1 (+5 as a second reason). **Zero of 139 reach for the bundle's *Deadline too close*, *Too small*, *Set-aside ineligible* or *Cost to pursue too high*.** Every chip carries a class on the way in (fit · evidence · notice · eligibility · capacity), per SVRC 1.1.4. **D20 is RULED — A, one chip for the 113, dictated by Matt in session 2026-09-16 ("one chip for the 111") and written into the store by Claude with a `recorded_by` field saying so.** D21 (the six Pass chips as derived) · D22 (Interested chips) · D23 (*Not enough information* on both branches) · D24 (a capacity chip the corpus did not earn, on SVRC 1.1.4's mandate) await him; the recommended option is first on each. **Two findings that are not chips:** spec §4.2 says the negative profile should be built from exactly these 113 no-bid reasons, and `firm_profile.negative_profile` is still empty; and an Interested row wearing *Not enough information* is precisely the row D2's on-demand document fetch exists for. Both are named on the sheet as follow-ups, not decided by it. The sheet's source is in the session scratchpad, not the repo, like every sheet before it. ⚠️ Verified on screen: header, facts, the rail reading `1 of 5 ruled` from the store, D20 rendered `Ruled · A` with its provenance, D21 with its options. The 150-block roster rendered without throwing (the rail updates after it) but its layout was NOT seen — the extension cannot scroll or read inside the artifact frame.

**⚖️ ALL FIVE ARE RULED — 2026-09-16, every one the recommended option, dictated in session (*"I have reviewed, and I am good with ALL of your recommendations"*) and written into the store by Claude with a `recorded_by` field saying so.** **D20 A** (one chip for the 113; the noun stays in the reason field) · **D21 A** (the six Pass chips as derived) · **D22 A** (the four Interested chips as derived) · **D23 A** (*Not enough information* on both branches, class evidence) · **D24 A** (*No capacity right now* ships with no corpus behind it, on SVRC 1.1.4's mandate). No block was moved and no chip renamed on the sheet's roster. **Plus a sixth, asked in chat the same day because it is a prototype-versus-ruling conflict under CLAUDE.md §1:** the bundle's guard (`!picked.length && !freeText.trim()` — a chip OR text) returns on both branches over D30's literal "free text required", **with one exception: `Not a service we provide` alone is not a decision**, because D20-A puts the category noun in the reason field and that noun is what seeds the negative profile. **Do not re-ask any of the six.**

**✅ BUILT 2026-09-16, TDD, on branch `build-d21-d24`. Gate **1173 tests / 104 files, `npm run check` exit 0** (from 1150 — twenty-three new, every one watched failing first). ~~Not merged, not pushed.~~ ✅ MERGED `--no-ff` at `60a7ba3` the same evening, on Matt's word, with the merged tree verified byte-identical to the gated branch tip; PUSHED with the closing docs commit.** Twenty-three new tests: 11 in `decide.test.ts` (chips stored and read back; a chip alone satisfies either branch; `not-a-service` alone refused with a sentence naming the chip; unknown chip and wrong-step chip refused before the database; the shared chip legal on both; duplicates collapse; New stores `[]`; the CHECK proved by a hand INSERT round the guard; the per-step lists pinned to the chips' own `branches`), 4 in `routes/triage.test.ts` (201 with chips echoed, 400 `reason_chips` for a stranger, 400 `reason` naming the chip, 400 for a non-array body), 8 new + 2 inverted in `Queue.test.tsx` (the seven Pass chips in order; twelve on Interested with the five reason chips under the second head; chip-alone confirms and the POST carries `reason_chips`; multi-select toggles; `Not a service we provide` blocked with "which service?" and then through with the noun; Back clears; the toast names the chips; the bundle's placeholder back on both branches). **The two no-chips pins are inverted, not deleted**, so the reversal shows in the diff.
- **Where it lives:** the vocabulary is `REASON_CHIPS` in `@tenderfoot/shared` — id, label, class, steps, the one `needsDetail`, and the bundle's word where it has one — with `reasonChipsFor(step)` giving each step's render order (largest cluster first; the shared chip is second on Pass and last on Interested). **Migration 035** adds `pursuit.reason_chips text[] NOT NULL DEFAULT '{}'`, a CHECK pinning the eleven ids, a GIN index. `recordDecision` validates vocabulary and step, dedupes, applies the guard and the D20-A exception, and stores `[]` on New; the route checks shape and answers 400 on `reason_chips`. `LATEST_PURSUIT` selects the column, so `decision.reason_chips` is on every latest row. The queue renders the row with the existing bundle-matched `ChoiceChip` in the existing `.queue__reason-chips` container on both steps, toggles, sends `reason_chips`, names the chips in the toast, and clears them on Back and on confirm.
- **Deviation D32** in `docs/admin-deviations.md` — the words are not the bundle's (zero of 139 reached for four of its eight), and the Interested step carries two chip rows where the bundle draws one. **D30's placeholder branch closes**: with chips back, the bundle's *"…or say it in your own words (this is the training signal)"* is right on both branches and is shipped verbatim. **SVRC Region 1.1.4** carries a dated *"Revised 2026-09-16 — chips return, derived"* paragraph.
- **What it does not do, on record:** the negative profile is still empty (its 113 nouns are now `WHERE reason_chips @> '{not-a-service}'` away — its own step, spec §4.2); an Interested wearing *Not enough information* does not yet trigger D2's fetch (D23 recorded it, nothing rules the trigger); the record screen renders no decision at all, so the chips wait there with the reason; nothing learns from reasons yet, and the first thing that does must filter `no-capacity` before reading a row (spec §1).
- **Mutation-proven, whole file each time, five mutations:** the server's D20-A detail guard deleted → 3 red across `decide.test.ts` and `routes/triage.test.ts`; the step-membership check dropped → 1 red; the dedupe dropped → 1 red; the client's detail guard deleted → 1 red; Back no longer clearing chips → 1 red. Each turned exactly the expected test and nothing else.
- **Click-through done (CLAUDE.md §4), through the Chrome extension on the LOCAL Windows Chrome, against the dev servers (`PORT=3010` / `VITE_API_TARGET=http://localhost:3010`, queue in `all` mode, 1,178 rows). Seen on screen, not just in the DOM:** the Pass step with `WHY NOT? — REQUIRED`, the seven chips wrapping to a second line in the bundle's flex-wrap container, and the bundle's placeholder; two chips selected at once in the accent fill (multi-select); `Not a service we provide` + `Pass & next` refused with *“Not a service we provide” needs the detail in the reason: which service?* in a callout under the field; Back; the Interested step with the channel row under `WHERE ELSE…` and the five reason chips under `WHY THIS ONE?`, nothing carried over. **No decision was written** — verified by a read-only query for pursuit rows in the last 30 minutes: none. The POST path rests on the route tests against a real schema.
- ⚠️ **A browser trap that cost ~20 minutes, now in CLAUDE.md §4:** the extension had TWO Chromes connected — this Windows one and a Mac — and was answering from the Mac, whose `localhost` is not this machine. Every `localhost` page read as an error page while `example.com` rendered fine. `list_connected_browsers` → `select_browser` on the `isLocal: true` one fixed it. The 09-15 "site permission" diagnosis was probably this same thing.
...
> ⚖️ **ALL THREE ARE RULED — 2026-09-15, and every one went to the recommended option.** **D15 A** (on demand) · **D16 A** (`DOCUMENTS NOT REQUESTED`) · **D17 C** (≤ 30% of biddable rows under 100 chars). **Do not re-ask any of them.** Matt dictated the three in session rather than clicking the sheets; each answer was written into its own page's store by Claude and carries a `recorded_by` field saying so, so the sheets now render `Ruled` exactly as if clicked. **None is built yet** — the build order is the numbered list directly below.
...
1. ~~**Build the rulings.**~~ ✅ **BUILT 2026-09-15, TDD, on branch `build-d15-d16-d17`. Gate 1150 tests / 104 files, `npm run check` exit 0** (from 1139 — eleven new tests, every one watched failing first). **Not merged, not pushed.**
   - **D15-A** — `DocumentClient` gained an optional `fetchKeyFor`; a keyless HigherGov row now buys its key by `source_id`, persists it in its own committed write, then fetches documents. Tallied under endpoint `opportunity` (already legal in migration 030's CHECK, so no migration). **The ceiling now prices TWO calls for a keyless open, not one** — otherwise an open could buy a key and then be unable to afford the documents it was bought for. `FetchOutcome.spent` is the whole operation's bill, and **`no-document-key` is no longer always free** — its doc comment says so.
   - **D16-A** — `DOCUMENTS NOT REQUESTED` plus a `record__doclist-why` reason line, deviation **D31**. 🔴 **NARROWED, and it is flagged for Matt:** the head shows only when the row is not-asked AND holds nothing. An IDOA row can be `unsupported` *with* real files (discover-idoa.ts writes documents and never stamps), and there is a pre-existing regression test pinning `BUNDLE — 1 FILE` for exactly that. D31 §"gated on holding nothing" has the argument; one condition to drop if he disagrees.
   - **D17-C** — `measureF6` measures the share of biddable rows under `thinDescriptionChars` (100) against `maxThinDescriptionShare` (0.3). `minDescriptionP10Chars` is gone. **Spec amended** at `docs/superpowers/specs/2026-09-03-data-fitness-and-source-rubric-design.md` §3.2, per CLAUDE.md §1, and §8.1's open question 1 closed. ⚠️ **It broke the R7 mirror**: `p6DescriptionP10Adequate` now mirrors nothing, and R7's p10 collapses on exactly the sources F6's did — **an unruled rubric defect, for Matt before R7 is ratified.**
   - **Click-through done (CLAUDE.md §4), at ZERO metered cost.** The extension could not reach localhost (site permission; it renders example.com fine), so CDP on a throwaway headless profile, by Matt's call. Three states seen on screen, not just in the DOM: IDOA 1970 → `DOCUMENTS NOT REQUESTED` / "no client for this source"; SAM 486 → `BUNDLE — 0 FILES`; SAM 202 → `BUNDLE — 7 FILES` with its seven rows. `api_spend` confirms **no spend in the last 3 hours.** The D31 narrowing could NOT be shown on screen — no live row is currently both unchecked and holding files — so it rests on its unit test.
   - **Mutation-proven, whole file each time:** F6's inclusive boundary, D16's not-asked guard, D15's two-call ceiling, and D15's key persistence. All four turned exactly the expected test red.
...
4. ~~Still waiting on Matt from before: a **dashboard reading** to settle whether the 400 on `api_spend` row 211 billed (then correct that row with evidence)~~ ✅ **READ 2026-09-15: 6,767, which matches our `opportunity` spend exactly and leaves row 211's 100 as the whole gap — it was never billed.** The correction of the row is on the D18 sheet. · Still open: the working-set plan's **§5**.
...
**Done 2026-09-13, by dictation, no shortcut.** F5's denominator exists: **150 decisions against HigherGov's multi-state data**, acceptance rate 7.3%. **The corpus is captured at [`docs/2026-09-13-reason-corpus.md`](docs/2026-09-13-reason-corpus.md)** — one block per decision with the card's facts beside the reason, grouped by branch, a blank `→ chip:` per block, and a mechanical word count labelled as a starting point and nothing more. Generated by `scripts/reason-corpus.mjs --sample=2`; re-runnable, read-only.

**▶️ NEXT: derive the chips from that corpus, with Matt** — the follow-up he named when he chose the full free-text run. Under SVRC 1.1.4 each chip carries a class on the way in, and the capacity class is excluded from anything that learns. ~~**Three ruling sheets are waiting for him first thing (his words: "first thing in the morning"): D15/D16, D17.**~~ ✅ **ALL RULED 2026-09-15 — D15 A · D16 A · D17 C, every one the recommended option. The build is what stands between here and the chips.** Links below.

**Matt reopened sample #2 on 2026-09-13 and worked through all 150 by dictation, in batches of ten.** The servers were up (`PORT=3010` / `VITE_API_TARGET=http://localhost:3010`, tab on `?sample=2`, header verified reading `SAMPLE · 150 of 1,755 · HigherGov · seed 1788890660565-4atkg6bi`).
...
**1. The document-fetch accounting fix — BUILT, GATE-GREEN, ~~ON A WORKTREE BRANCH, NOT YET MERGED~~ ✅ MERGED `--no-ff` at `927b1e1` once Matt said he was done, then hardened at `f5b0ad5`.** Branch `worktree-doc-fetch-accounting` (eight commits from `d07ce75`, now deleted). Gate on the merged result **1139 tests / 104 files, exit 0**. ⚠️ **`main` is 10 commits ahead of `origin/main` and NOT pushed** — Claude said it would ask rather than assume; ask Matt first thing. *(The paragraph below is as written mid-session.)* **Deliberately not merged while Matt is triaging** — the dev server watches `app/server/src`, and a merge into `main` would restart the API under him mid-decision. **Merge `--no-ff` the moment he is done.**

🔴 **ONE THING THE ISOLATION DID NOT COVER, found after the merge:** `npm run check`'s build step runs `migrate:deploy` against `DATABASE_URL_TEST`'s **public** schema, and `DATABASE_URL_TEST` names the **same Neon branch** the dev server reads (`ep-withered-base`; `scripts/check.mjs`'s own comment says so). So the worktree's gate applied **migration 034 to the live triage database at 01:37Z, mid-sitting** — a nullable column, no data touched, the running server did not select it until the merge, and the 150 ran to completion unaffected. Harmless this time; recorded because "tests run in throwaway schemas" was true of the tests and not of the gate, and a locking or destructive migration gated the same way would not have been harmless. A second consequence: a gate run from the main checkout with the merged worktree still on disk ran **208 test files instead of 104** — vitest globbed `.claude/worktrees/` and the two copies collided on schema names, 86 failures for nothing. `f5b0ad5` excludes `.claude/` from vitest and gitignores it.
...
🔴 ~~**UNTIL THE MERGE, THE RUNNING SERVER STILL HAS THE OLD CODE**~~ ✅ **MERGED; the dev server restarted on the merged code and answers 200. Opening a HigherGov record is now free (it is refused for lack of a key, `no-document-key`, until D15 is ruled).** *As written mid-session:* **— and the trigger is not a button. The record screen fetches documents automatically the first time an unchecked row is OPENED, so "Open record" on a HigherGov row is what costs the phantom 100.** Verified read-only mid-session: **0 `api_spend` rows today**, so nothing has landed. After the merge an open is free, and the *reason* it failed is now known and fixed — see the next section — though a row ingested before tonight still cannot fetch until D15 is ruled. **The phantom 100 in `api_spend` id 211 is left in place** pending Matt's dashboard reading, as before.
...
**3. `maxCallsPerRun` — MEASURED, THE QUESTION CHANGED, AND ⚖️ BOTH RULINGS ARE IN (2026-09-13, ~21:09 local). Ruling sheet D13–D14: <https://claude.ai/code/artifact/11570fba-9908-496f-b5d8-45db5a2b27bf>** (answers in its own store, `read_db`, collection `rulings`, docs `d13`, `d14`). **Do not re-ask either.**

| | Ruled | Built |
|---|---|---|
| **D13** — the number | **A — keep 500, re-state why** | ✅ The value is untouched. `thresholds.ts`'s comment on `maxCallsPerRun` now strikes the archive-walk reasoning and carries the measured facts and what 500 actually is (the recall run's runaway-sweep stop, 2–4× its real need); `MAX_CALLS_PER_RUN_RATIFIED` stays `true` with a re-ratification note; `coverage-cli.ts`'s printed line names D13; `run.ts`'s "question for Matt" paragraph is answered in place; `thresholds.test.ts` still pins 500 |
| **D14** — should the ingest walk answer to a call cap at all? | **A — no, leave it as designed** | ✅ No behaviour change. `highergov-cli.ts`'s header records that `--max-records` is optional *by ruling*, and that this command answers to no call cap, so the next reader does not mistake it for an omission |

Two facts before re-asking, which are what the ruling was made on: **the cap is enforced in ONE place, `npm run recall` — the ingest CLI that walked the archive (the walk 500 was sized for) has no call counter at all**, its guards being the monthly ceiling and the *optional* `--max-records`. And from the 379 day-artifacts in `runs/`: the largest run ever was **132 calls** (the Jan–Jun sweep, 0 records — precisely the zero-result exposure the cap exists for), the paged runs averaged **1.06 calls/day** with a worst day of **4 pages**, and every live recall run stopped on `maxRecordsPerRun` (40) first. **D13** asks the number (recommended: keep 500, re-stated as the recall run's runaway-sweep stop — `thresholds.ts` currently cites the wrong walk). **D14** asks whether the ingest walk should answer to a call cap at all (recommended: no change). Lesson 2.33.

### 🔑 THE 400 IS EXPLAINED, AND THE FIX IS BUILT — BUT NO EXISTING ROW CAN USE IT WITHOUT A RULING

**Found 2026-09-13, from the vendor's public OpenAPI schema (`highergov.com/api-external/schema/`, no key, zero records).** `/document/` accepts exactly five parameters — `api_key`, **`related_key`**, `ordering`, `page_number`, `page_size` — and the endpoint's own description says: *"The related_key is required and is found in the document_path field in the Opportunity endpoint."* We sent `source_id`, which it does not accept. Every `HigherGov answered 400` was a rejected request; the key was never in question.

**Why no row could ever have made the call:** `document_path` embeds the api_key, so the client drops it at parse time (correctly, §5.3) — **without lifting `related_key` out of it first.** The artifacts on disk were written after that parse, so the key is nowhere: **all 3,238 HigherGov rows are keyless.**

**Built on the worktree branch (same one as the accounting fix), TDD, gate-green:** the client reads the one parameter out of `document_path` before dropping it (`FeedNotice.documentKey`, and `raw.document_key` so it rides the artifact and the merge like `pop_state`); **migration 034** adds `solicitation.document_key`; `merge/document-key.ts` lands it on both the creation and update paths, with a `documentKeysSet` counter; `DocumentClient` declares `keyedBy` (`external-id` for SAM, `document-key` for HigherGov) and `fetchDocumentsFor` hands the right column to the client; `/document/` is asked by `related_key`. **A keyless HigherGov row is refused before any request — `reason: "no-document-key"`, 0 spent, unstamped** — rather than guessing and paying for the 400. The old client test that pinned `source_id` is retired with a note. ~~⚠️ Migration 034 is on the branch only; **`npm run migrate` against the dev database after the merge**~~ ✅ Applied to the dev database — by the worktree's own gate at 01:37Z, before the merge, which is the finding recorded under task 1 above. `npm run migrate` after the merge reported "no pending migrations", and `public.solicitation.document_key` is present, verified by query.

⚖️ **Ruling sheet D15–D16: <https://claude.ai/code/artifact/87cdf0b6-aca8-4274-b8ec-49da0d0e9fff>** (answers in its own store, `read_db`, collection `rulings`, docs `d15`, `d16`). **D15** — how the rows already in get a key: on demand (+1–2 records the first time a keyless row is opened, then ~11 as before; recommended) · backfill the 150-sample now (150–300 records, mostly on rows that will be passed) · backfill only the Interested rows after the sitting (~15–30) · nothing. Every price assumes the trial's 2,233 are still spendable. **D16** — what the record screen says for a row it could not ask about: today `unsupported`, `ceiling` and now `no-document-key` all render `BUNDLE — 0 FILES`, which is D3's error on screen; the bundle has no "could not ask" state, so it is a §7.10 question (recommended: one invented head, `DOCUMENTS NOT REQUESTED`, with the reason beneath). ~~**Neither is built; both are for Matt.**~~ ⚖️ **BOTH RULED 2026-09-15 — D15 A (on demand), D16 A (`DOCUMENTS NOT REQUESTED`), each the recommended option. Do not re-ask. Neither is built yet.**
...
⚖️ **Ruling sheet D17: <https://claude.ai/code/artifact/4654f19d-f3d3-4c60-b63c-edbfc8b21a72>** (`read_db`, collection `rulings`, doc `d17`). Options: keep `p10 ≥ 200` (permanent fail, floor cannot pass) · lower to 100 (still fails — p10 29/68) · **change the statistic to "at most 30% of biddable rows under 100 chars"** (HigherGov 27.8%, SAM 14.0%, combined ≈22% — passes; the sheet says plainly that 30% is set to the market as found; recommended) · retire F6 from the floor and leave description length to R7. ~~**Changes a predicate Matt ratified in D4, so it is his.**~~ ⚖️ **RULED 2026-09-15 — C, the recommended option: F6 becomes "at most 30% of biddable rows under 100 chars". Do not re-ask. Not built yet, and because it amends a D4-ratified predicate the spec amendment goes with the build, per CLAUDE.md §1.**

### 🔍 THE BRANCH WAS CODE-REVIEWED BEFORE THE MERGE, AND SIX OF NINE FINDINGS WERE BUILT

`/code-review` against `main`, medium effort, on the seven-commit branch. **Built:** a refusal is a **4xx only** — a 5xx (502/504 from a gateway) can arrive after the origin served and billed, the same unknowability as a transport failure, so it keeps the estimate (the one measured "errors do not bill" observation is a 400) · one exported `httpStatusOf()` read by both the page-walk wrap and the pricing, so they cannot drift · **`keylessPaths`** — the client counts rows whose `document_path` carried no `related_key`, the envelope carries it, and the ingest prints it per day and in the summary, because the parameter name inside `document_path` is what the schema implies and **no fixture confirms the live shape**: a mismatch must show on the first live run, before D15 spends anything on the assumption · the reason-corpus script no longer loses a `Triaged` row between its counts, refuses a bare `--out`, and drops an unused column · the test stub carries a status · the over-promising comment in `fetch-documents-for.ts` now says plainly that no caller consumes `no-document-key` yet (D16). **Declined, with the reason:** dropping `FeedNotice.documentKey` in favour of `raw.document_key` alone — the file's own convention gives raw-derived fields a named twin (`postedDate`'s comment says why), and this follows it. **Contingent on a measurement, not built:** `document_key` NULL conflates "never captured" with "the vendor sent a path with no key"; whether the second case exists at all is what `keylessPaths` measures on the first post-merge ingest, and a sentinel is designed only if it reads non-zero.

**Also this session:** the admin secret was copied to Matt's clipboard on request, without display.

> ⚖️ **RECONCILED 2026-09-15 — THE PHANTOM 100 WAS NEVER BILLED, and the arithmetic is exact.** Matt's dashboard audit reads **6,767**. This month's ledger splits as `opportunity` **6,767** records / 207 calls and `document` **100** / 1 call — so the dashboard equals our opportunity spend *to the record*, and the single `document` row is the entire discrepancy. That row is **`api_spend` 211** (100 records, 2026-09-08, solicitation 2422), the conservative bound charged for a 400 refusal before the 09-13 fix. **A refused request costs nothing is now MEASURED, not argued.** The correction of row 211 itself is proposed on the D18 sheet rather than written silently — it is the one instrument we have for spend, and rewriting it unasked is not on.
>
> ✅ **D18-A AND D19-A RULED AND EXECUTED 2026-09-15 — 49 RECORDS, AND HIGHERGOV LOOKS GOOD.** Matt took both recommendations. Spend went **6,867 → 6,916**; ceiling headroom **2,084**, allowance headroom **3,084**.
>
> **D18-A — the `related_key` assumption HOLDS, and the proof is end-to-end.** The dry run cost 5 and bought only a projection (`--dry-run` stops before the walk, so it never reaches the `KEYLESS PATHS` line — worth knowing before anyone buys that answer again). The real one-day walk of 2026-09-02 cost 6 and settled it: the artifact carries `document_key` with real values, `document_path` correctly stripped, `keylessPaths: 0` truthful. 🔴 **THE OPERATIONAL TRAP: `npm run ingest:highergov` DOES NOT MERGE.** Keys sat in `sighting` and `solicitation.document_key` read 0 until `npm run merge` ran — after which 6 rows were keyed and the total moved 3,238 → 3,240. The first reading looked exactly like a broken parse and was a missing step.
>
> **D19-A, half one — all three still-open Interested rows now hold their documents, for 18 records.** 4794 SBO Medicaid (5 records: 1 key + 4 docs), 2244 Safe Streets (2: 1 + 1), 2121 Indiana Medicaid MCOs (11: 1 + 10). **Every document arrived with text already extracted.** ⚖️ **THE ~11-RECORD ESTIMATE IS WRONG AND THE SHAPE IS BETTER THAN BUDGETED: the real cost is `1 + N documents`**, so a thin notice costs 2 and a fat one 11. The 11-record row is the only one that hit 10 documents, and §5.2's "10 of 19" warning means that bundle may be page one of more — the client does not page, by design.
>
> 📈 **D19-A, half two — THE RECALL MEASUREMENT, AND IT IS THE COMMERCIAL HEADLINE.** `npm run recall --from=2026-09-01 --to=2026-09-15`, 66 free IDOA notices as the answer key, **20 records**. Cohort now **77 settled**. **C1 coverage recall 0.987 against a 0.95 threshold. C2 timely recall 0.987 against 0.90. C3 capture latency: median 39 days of bidding time still left when HigherGov first carried it.** Both gates would PASS — they report `unknown` only because **C4 (cohort ≥ 100) reads 77 and FAILS**. ▶️ **23 more settled notices is the whole distance between here and a graded verdict on whether to buy this source.**
>
> 🛑 **THE NARROW WINDOWS ARE DONE, AND C4 CANNOT BE REACHED AT ANY PRICE (2026-09-15, Matt approved "run the narrow windows").** Four more runs — 09-03→04 (10, aborted), 09-04→08 (28, clean), 09-09→15 (14, aborted on 09-10), 09-11→15 (19, clean). **71 records; cohort 77 → 82, then flat.** Spend **6,916 → 6,987**.
>
> **Why it stopped growing, measured from `coverage_item` at zero cost: only 82 distinct notices have EVER been observed, and IDOA currently lists 66.** The cohort is bounded by the size of the free answer key, not by the API budget. **C4's floor of 100 is therefore unreachable before the trial ends, for any number of records** — more runs cannot help, and that is why no more were bought.
>
> 🔴 **AND THE HEADLINE HAS A CAVEAT THAT MATTERS MORE THAN THE NUMBER: all 82 notices are `state_agency`. `sub_state` reads NOT MEASURED — zero notices, every run.** CLAUDE.md §5.2 says plainly that sub-state is "the segment this source is bought for". **So the trial measured HigherGov on the segment we did not buy it for, and is silent on the one we did** — because the free answer key (IDOA) is itself a state-agency source, and we have no free sub-state source to measure against. That is a structural gap in the measurement, not a gap in the spending.
>
> **What the trial DOES support, stated for the subscription decision:** on state agencies, **98.7%–100% coverage recall**, **98.7%–100% timely recall**, **37–39 days median bidding time remaining** when HigherGov first carries a notice. Ungraded (C4), and unmeasured on sub-state.
>
...
>
> ~~**1. The chip derivation, WITH MATT**~~ ✅ **DONE 2026-09-16 — derived, ruled (D20–D24 + the guard), built, merged. See the block at the top of this file.** *(As written:)* **from [`docs/2026-09-13-reason-corpus.md`](docs/2026-09-13-reason-corpus.md).** 150 blocks, 11 Interested, a blank `→ chip:` per block. No API calls, no rulings outstanding, and it is the thing everything else has been waiting on. Under SVRC 1.1.4 each chip carries a class on the way in and the **capacity class is excluded from anything that learns**. Then unpin `Queue.test.tsx`'s no-chips assertion, record the deviation, and build the chips with the free-text escape hatch the SVRC requires. *(If the 150 were touched again since, re-run `node --env-file-if-exists=.env scripts/reason-corpus.mjs --sample=2 --out=docs/2026-09-13-reason-corpus.md` first — read-only and idempotent.)*
>
...
✅ **MERGED to `main` `--no-ff` at `c6e3953`, 2026-09-07, on Matt's approval.** 33 commits, 47 files, +4,401/−60. **Gate re-run ON THE MERGED RESULT: 1,019 tests / 104 files, exit 0** — the identical figure the branch carried, so the merge introduced nothing. The branch gate was also re-run by the controller on the real tip rather than taken on the implementer's report, which had described it as run *before* its own commits on equivalent content. ⚠️ **`main` is ahead of `origin/main` and NOT YET PUSHED.**
...
⛔ **IT REFUSES ON A FRESH DATABASE — and as of 2026-09-07 exactly ONE of the two preconditions is still open.** Migration 019 seeds the source `enabled = false` ("Nothing is ever seeded on") and **no migration flips it**, so the command dies at `resolve-source.ts:120` — *"Source 'HigherGov' is disabled … a disabled source is refused, not silently skipped."* **That one is still open, and enabling it is a deliberate operator act.** ~~And `HIGHERGOV_SEARCH_ID` is unset~~ ✅ **`HIGHERGOV_SEARCH_ID` was supplied by Matt and written to `.env` on 2026-09-06** — it is the ONLY Indiana filter this API has, and `searchId()` throws without it by design, because a missing scope means a NATIONAL pull billed in full.

> ℹ️ **Which database a local run would touch, checked rather than assumed 2026-09-07:** `DATABASE_URL` and `DATABASE_URL_TEST` are currently **byte-identical strings** in `.env`, both naming the `ep-withered-base-au6l4cjf` compute — the **test** branch. So no local operator command can reach production as things stand, and CLAUDE.md §2's "check which branch you are pointed at" has a trivially safe answer *today*. It will stop being trivially safe the moment those two strings differ, which is the point of checking rather than remembering.
...
**✅ MERGED 2026-09-05 at `d60ae40`, `--no-ff`** — the sentence above was written hours before the merge it describes, and is corrected 2026-09-06. **`--no-ff` is verified rather than remembered:** main's tip was an ancestor of the branch tip, so a fast-forward was available and was not taken.

**Gate re-run on the MERGED result, 2026-09-06: 825 tests / 93 files, exit 0.** That is the same figure the branch carried, which is the point of re-running it — the merge introduced nothing. `main == origin/main`, working tree clean.
...
`STATUS.md` here · `DOOGIE - TENDERFOOT.md` session log · `docs/Tenderfoot-Plan-of-Action.md` the sequence and its reasoning · `docs/Proto2PRD.md` the reusable playbook · `docs/Proto2PRD-Lessons.md` lessons staged for it · `docs/superpowers/specs/` design + workflow specs · `docs/superpowers/plans/` implementation plans · `reference/` SVRC + component inventory · `prototype/` frozen, reference-only · `corpus/` real solicitations and contracts · `docs/explainer/` the explainer (2026-08-12, internal until rebuilt) · `docs/addendum/` the dated management addendum behind it (2026-09-16; its README says when to rebuild)
