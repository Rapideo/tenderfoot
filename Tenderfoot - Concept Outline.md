...
| 0B | **Firm Profile** | The only home for customer-specific facts (§4.2): capabilities, codes, certifications, geography, hard limits, past performance, negative profile, capacity timeline. | 0A | ✅ |
| 0C | **Source Registry** | Sources as rows, not code. Carries tier, **platform** (§5.7), legal posture (§5.5), archive depth, yield baselines. | 0A | ✅ |
| 0D | **Storage + migrations** | Stack undecided (§10.5). | — | ✅ |

**Capacity timeline (0B)** is worth calling out separately: it is not a number but a
schedule of when current engagements end. Timing scoring reads it (3D) and the Phase 0
coverage report compares against it (5C). It is the one Profile field with an open question
attached (§10.3).
...
| 3D | **Stage 2 — LLM adjudication** | Survivors only. Produces the four machine scores — Fit, Winnability, Value, Timing — kept separate and separately visible (§6.3). **Timing is weighted against the capacity timeline** (0B), because the objective is utilization. The brief is a byproduct of the same call. | 3C, 0B | ✅ |
...
| 4B | **Pursuit lifecycle** | `New → Triaged → Watching → Bid/No-Bid → Submitted → Won/Lost`. Every transition carries a **reason** — and with no prior history to seed it, that field is load-bearing from decision one (§4.5). | 0A | ◐ |
| 4C | **Pipeline board** | Pursuits in flight. | 4B | ✕ |
...
| 4H | **Reports** | Phase 0 market sizing as a live view, including the coverage-against-capacity chart (5C). Source yield. Win rate only once history exists — never a system objective (§8.6). | 5C | ✕ |
...
| 5C | **Coverage report** | Adjudicated results grouped by month against the capacity timeline. Coverage, not volume: ten opportunities in one quarter is worse than five spread across the year. **Value-weighted, never count-weighted** — KP engagements span roughly 50×. | 5B, 0B | ✅ |
...
Reaching the coverage report also requires the rest of the ✅ set: sightings and dedup (2G),
