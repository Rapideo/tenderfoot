...
No deviation — faithfully transcribed. **See Finding F1 (§4)**: `rot`=yellow/`failing`=red is counter-intuitive by name; not a fidelity defect. Non-visual accessibility (`role="img"`, `aria-label`, `title`, `data-state`) added — acceptable without paperwork per §7.10.
...
Bundle anchor: the DEADLINE/EST. VALUE/POSTED trio, **index ~556698**. Label reuses `MicroLabel` (byte-identical declaration). Value: `--type-metric-stat` (purpose-named, 4 uses). Sub: `--type-body-detail` (11 uses, the single highest-use BODY-family token). `emphasis` is a boolean, not a colour prop, because across all 5 mock records `deadlineColor` only ever takes `var(--text)` or `var(--warn)` [REPORT — extracted all 5 values directly, not independently re-run by this audit].
No deviation.
...
Bundle anchor: the persistent footer, the only `<footer>` in the bundle, **index ~617384**. Full declaration transcribed in `StatusBar.tsx`'s own header comment. Cross-checked against the Source Registry's 5-entry mock array: 4 sources counted (GovWin IQ excluded, "Not ingested"), 1 failing (Ohio Procurement, "Failing"), 1 rot suspected (SAM.gov) — the footer's literal counts reconcile exactly [REPORT; not independently re-summed by this audit].
**Deviation D11** (mandatory, verbatim): the dot colour is a fixed `--signal-watch` literal, not derived from `failing`/`rotSuspected` — the bundle has one instance and it is never conditional (`background:var(--yellow)` is a static literal in the bundle, confirmed [REPORT], not a template binding).
...
> **PARTIALLY CLOSED 2026-08-15 — the primitive exists; the composition does not.** `Section` (`app/client/src/primitives/Section.{tsx,css}`, 5 tests) supplies the shared `padding:20px 30px 24px` with `recessed` and `divider` as **independent** modifiers — the bundle shows one apiece and never both, so collapsing them into a single "recessed" variant would invent a pairing it does not evidence. `--ground-recess-1` went from zero consumers to one.
>
> **Two of the three instances are covered; the third is deliberately excluded.** `HeaderLockup`'s `min-width:150px;flex:none` is a flex-item sizing constraint on a lockup inside a nav row, not section chrome — it belongs to whatever header container gets built, and this audit already notes the gap is moot until such a row exists. Folding it in would give one primitive two unrelated jobs.
>
> **What stays open, and it is the half that matters visually.** The gallery's existing `FactPanel` and `ScoreStrip` entries were **not** rewired to sit inside `Section`. That is a composition decision, SP6 is the slice that composes, and SP2 is signed off — re-dressing reviewed output in passing would change what was signed off without anyone looking at it again. `Section` is shown in its own gallery block beside a `FactPanel` so the difference is judgeable side by side. **D6 is not fully closed until a composed screen uses it.**
...
**F1 — `StatusDot`: `rot` = yellow, `failing` = red.** Faithfully transcribed from the Source Registry mock data ("Rot suspected"→`--yellow`, "Failing"→`--bad`); possibly backwards in the prototype itself. `StatusBar`'s single dot instance is always `--signal-watch` (yellow) regardless, so this only bears on `StatusDot`.
