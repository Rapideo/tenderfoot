...
function firstDocxWithTable(): Buffer {
  const files = globSync("**/*.docx", { cwd: CORPUS });
  for (const f of files) {
    const b = Buffer.from(readFileSync(new URL(f, CORPUS)));
    if (b.includes(Buffer.from("w:tbl"))) return b;
  }
  throw new Error("no .docx with a table in corpus/");
}
...
test("uses convertToHtml so table structure survives", async () => {
  ...
  const r = await parseDocx(firstDocxWithTable());
  expect(r.kind).toBe("text");
  if (r.kind !== "text") return;
