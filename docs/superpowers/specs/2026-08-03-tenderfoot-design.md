...
| Michigan SIGMA VSS | CGI Advantage VSS | ⚠️ **Renders without login, but the terms forbid it** — see §5.8. Legal posture, not technical access, is the blocker | **Unproven, and probing stopped** |
| Kentucky eMARS VSS | CGI Advantage VSS — *same as Michigan* | ⚠️ Same platform — **assume the same restriction until checked** | Unproven |
...
| **Michigan SIGMA VSS (CGI Advantage)** | Pages render with no login — *View Published Solicitations* and *Award History* both return data, the latter 3,762 records. **But the system displays an access-restriction banner:** *"intended for **government authorized users only** for use in conducting government business ONLY… **Disconnect immediately if you do not have express written authorization to access SIGMA!**"* | ⚠️ **Legal posture, and it outranks the technical finding.** Probing stopped on sight. See the note below |
| **Michigan SIGMA VSS** | Result totals are withheld — the grid reports `1 - 20 of 20+ Records` rather than a count. The `Show Me` filter *does* apply (the control's value changed `Open` → `All`), but with no total there is nothing to compare, so §5.4's vary-a-parameter method cannot run here | Closed-solicitation retention is **unproven and will stay unproven** unless the access question is resolved first |
...
1. ~~**Do the Ohio, Michigan, and Kentucky portals allow anonymous browsing?**~~ **Answered 2026-08-12 — and the answer for two of the three is a legal one, not a technical one.**
   ...
   **Michigan: pages render without a login, and the system's own terms say they should not be read by us.** SIGMA displays *"intended for government authorized users only… Disconnect immediately if you do not have express written authorization to access SIGMA!"* **Probing stopped there.**

   > **The wording is unambiguous; its scope is not.** VSS stands for *Vendor* Self Service and exists for vendors, so a banner addressed to "government authorized users" may be boilerplate inherited from the wider SIGMA financial system rather than a restriction meant for the public vendor pages. **That ambiguity is the finding.** It is a question for a person to resolve — with the State if necessary — and not one an engineer should settle by assuming the convenient reading. §5.5 makes legal posture a first-class Registry field precisely so this is recorded rather than guessed.

   **Kentucky runs the same platform and should be assumed to carry the same restriction until someone checks.** So the platform-adapter leverage in §5.7 is intact technically and **blocked on a legal question covering two states.**
