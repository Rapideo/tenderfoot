...
5. **Does the rendering get rebuilt repo-native, or does iteration stay in Claude Design with a re-extract each round?** Raised 2026-08-10 by the first extraction, and **partly answered within hours by V1.1.** Re-extraction is not free and is not one-off: every Design iteration invalidates it. But the cost is narrower than feared — the generator closed the token gap by itself, so only the mock layer genuinely needs re-extracting, and only its **comments** must be carried forward by hand. Measured in `ClaudeDesign_Proto_Cleanup.md`. The question that remains is whether the visual artifact ever becomes the specification, as IMPACT's was, or stays disposable with `src/app.js` carrying the spec. See §A7.
