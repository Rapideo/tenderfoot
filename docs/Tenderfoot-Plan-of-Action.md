...
2. **The third option still stands, and is now the whole of the near-term work.** ✅ **DONE 2026-09-03 — 204,920 contracts loaded, merged as `bd80e45`.** Real contracts now sit in the real schema beside real solicitations, and the Organization ↔ Contract half of the chain is exercised (`org_id` populated on every row; the organization registry grew 698 → 825). **Vendor resolution is NOT exercised** — `vendorName` lands as free text in `source_note`, and resolving `TIMOTHY WARRICK` to `Timothy Warrick, Inc.` remains its own slice. Fold the contract register into **SP1's demo criterion**: real contracts load into the real schema beside real solicitations. That tests Organization ↔ Vendor ↔ Contract early, costs nothing extra, and moves no post-gate feature forward.
