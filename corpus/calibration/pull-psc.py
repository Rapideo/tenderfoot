...
PSC = ["R408","R410","R422","R499","R699","R707","U008","U099","B506","B599"]

TYPES = ["o","k"]              # Solicitation, Combined Synopsis/Solicitation
BASE = ("https://sam.gov/api/prod/sgs/v1/search?index=opp&size=100"
        "&sort=-modifiedDate&is_active=false&psc={n}&notice_type={t}&page={p}")
        ...
        for p in range(40):
            ...
            for x in rs:
                ...
                rows.append({
                    ...
                    "psc_codes": [c.get("code") for c in (x.get("psc") or []) if c.get("code")],
                    "psc_q": n,
                    "agency": org(x),
                    "pop": pop(x),
...
json.dump(rows, open(os.path.join(SP, "archive_psc.json"), "w", encoding="utf-8"),
          ensure_ascii=False)
